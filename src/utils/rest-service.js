...
export class RestService {
  ...
  post(endpoint, data, header) {
    ...
    return promise
      ...
      .then((result) => {
        this.publish(promise);
        if (result) return this.parseResult(result);
  ...
  delete(endpoint, data, header) {
    ...
    return promise
      .catch((e) => {
        if (e.status === 404) {
          return Promise.reject(new Error("Data tidak ditemukan (404)"));
        }
        return e
          .json()
          .then((result) => {
    ...
    var request = {
      ...
      headers: new Headers(
        Object.assign({}, this.header, header, {
          ...
          "x-timezone-offset":
            this.endpoint.defaults.headers["x-timezone-offset"],
        })
      ),
    };
